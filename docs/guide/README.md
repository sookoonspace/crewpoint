...
2. **Web reloads always re-fetch from Firestore and lose any queued writes.** Firestore web persistence is intentionally OFF (audit V1 launch blocker #6, founder-acknowledged). Affects every browser and every mode. The Drift mirror on web is in-memory Wasm so cold-start has nothing cached.
3. **No offline writes anywhere (mobile or web).** Submissions require a network — error snackbar fires on failure; no queue, no retry. This is intentional per Pillar 1 design.
