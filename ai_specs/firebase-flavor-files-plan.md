...
- [x] Run `flutterfire configure` three times with `--platforms=android` to capture each file:
...
- [x] Remove `android/app/google-services.json` from root (it should only exist in flavor dirs)
- [x] Verify: `flutter build apk --flavor dev` and `flutter build apk --flavor prod` both resolve correct google-services

### Phase 2: iOS Per-Flavor GoogleService-Info.plist

- **Goal**: Each iOS flavor gets the correct plist at build time
- [x] Capture each flavor's plist:
...
- [x] Add Xcode Run Script build phase to copy correct plist:
  ```bash
  ...
  ```
- [x] Update `ai_specs/setup-guide.md` Section 2 to re-add the plist copy phase with the new paths
- [x] Verify: `flutter build ios --flavor dev` picks up dev plist

### Phase 3: Activate Dart Firebase Options

- **Goal**: Replace placeholder in `firebase_service.dart` with real generated imports
- [x] `lib/app/core/services/firebase_service.dart` — import all three generated files; select by `AppFlavor`:
  ```dart
  ...
  ```
- [x] Remove placeholder `FirebaseOptions` block and TODO comment
- [x] Verify: `flutter analyze` && `flutter test`

### Phase 4: Update Setup Guide

- **Goal**: Guide reflects actual working approach
- [x] `ai_specs/setup-guide.md` — Update Section 1.3 to document the capture-and-move workflow for native files
- [x] `ai_specs/setup-guide.md` — Re-add Section 2.4 (plist copy phase) with `ios/Runner/Firebase/{flavor}/` paths
- [x] `ai_specs/setup-guide.md` — Note that Android uses `android/app/src/{flavor}/google-services.json` (Gradle handles it automatically)
- [x] `ai_specs/setup-guide.md` — Update Section 10 (Common Issues) for new file paths
