...
- [ ] `lib/app/core/format/event_date_range.dart` - new pure function `formatEventDateRange(DateTime? start, DateTime? end, {DateTime? now})`. Returns `'MMM d – MMM d'` (en-dash), appends `', yyyy'` only when range spans years OR year ≠ `now.year`. Null/partial: see spec req 12.
- [ ] TDD: same-year both set → `'May 22 – Sep 30'`.
- [ ] TDD: cross-year → `'May 22, 2026 – Jan 5, 2027'`.
- [ ] TDD: same-day collapse → `'May 22'`.
- [ ] TDD: null end → `'May 22'` (single endpoint).
- [ ] TDD: null both → empty string.
- [ ] TDD: `now.year != range.year` → year appended on both ends.
- [ ] `lib/app/core/widgets/event_tile.dart:28-35` - replace `_dateRange()` body with `formatEventDateRange(event.startDate, event.endDate, now: DateTime.now())`.
- [ ] `lib/app/features/dashboard/presentation/event_dashboard_screen.dart:153,277-305` - delete `dateFormat` + Row+Text+Text+Text rendering; replace with single `Text(formatEventDateRange(...))` retaining the calendar Icon + spacing.
- [ ] Update existing event_tile date-row test (if present) — locate via Grep `_dateRange|May 22` in `test/`.
