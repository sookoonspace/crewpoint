...
- [x] `web/firebase-messaging-sw.js` — service worker using `firebase-{app,messaging}-compat 10.13.2`. `firebase.messaging()` with the default `onBackgroundMessage` handler renders the `notification` payload as a system notification — sufficient for V1. Project config is hardcoded to `crewpoint-dev` (matches `lib/firebase_options.dart` default); the file's header doc lists the stg/prod field swaps for deploy.
- [x] `lib/app/core/services/fcm_service.dart` — gains `String? platform` + `String? vapidKey` constructor args. `_platform` defaults to `kIsWeb ? 'web' : 'mobile'`. `attach()` threads `_vapidKey` into `gateway.getToken(vapidKey: _vapidKey)`; `_platform` flows into every `addFcmToken` (initial + refresh listener) and the matching `removeFcmToken` in `detach()`. `IFcmGateway.getToken` signature now `Future<String?> getToken({String? vapidKey})`. `lib/app/core/providers.dart` reads `--dart-define=FIREBASE_VAPID_KEY=...`; empty string → null so mobile builds pass nothing.
- [x] **Token-schema migration** — `IUserRepository.addFcmToken` / `removeFcmToken` now require `platform`. `FirestoreUserRepository` writes `{value, platform}` objects to `users/{uid}/private/profile.fcmTokens`. CF `extractTokenValue(rawToken)` helper reads BOTH legacy plain strings AND new `{value, platform}` objects; `TokenOwner.rawToken` preserves the original shape so `pruneDeadTokens` `arrayRemove`s the exact stored entry. No data migration needed — legacy tokens stay readable until they expire / fail and get pruned.
- [x] TDD: web token registration stores `{platform:'web'}` (`fcm_service_test.dart` "explicit platform: 'web' propagates to both initial + refresh writes" + `firestore_user_repository_test.dart` "addFcmToken tags web tokens with platform 'web'"); android/ios stores `{platform:'mobile'}` (default-constructor test + "addFcmToken appends a {value, platform} object to the private subdoc"); legacy plain-string tokens still readable (`sendPush.test.ts` 3 new `extractTokenValue` tests cover string / object / malformed shapes); `attach()` threads vapidKey through to the gateway.
- [x] Verify: `flutter analyze` && `flutter test` && `npm --prefix functions test`. *(1 pre-existing TableMigration warning; 808 flutter tests pass, 4 skipped; 157 CF tests pass — +3 across `extractTokenValue`.)*
- [ ] **Manual**: web build smoke — `flutter run -d chrome` with `--dart-define=FIREBASE_VAPID_KEY=<vapid>`. Confirm: (a) browser prompts for notification permission, (b) token written to `users/{uid}/private/profile.fcmTokens` as `{value, platform: 'web'}`, (c) background push fires a system notification with the service worker registered, (d) tap routes to the deep-linked page. Per-flavor `firebase-messaging-sw.js` swap covered separately at deploy time. *(Out of session scope.)*
